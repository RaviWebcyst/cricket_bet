<template lang="">
                    ...
                    <div>
                        ...
                        <Spinner name="moon" color="#c6172c" style="text-align:center;position:relative;top:200px;" v-if="loading==true" />
                            ...
                            <h6 class="mb-4 ">Balance: <small class="bal">{{balance.toFixed(2)}}</small></h6>
                            <div>
                                <table class="table mt-1">
                                    ...
                                    <tbody v-if="runners.length > 0">
                                        <tr>
                                            <td>{{a}}<span class="team_a"></span></td>
                                            <td>  ({{wallet_balance_a.toFixed(2)}})</td>
                                            <td :id="'rt_'+back" @click="rate(back,1)">{{back}}</td>
                                            <td :id="'rt_'+lay" @click="rate(lay,3)">{{lay}}</td>
                                        </tr>
                                    </tbody>
                                    
                                    <tbody v-if="runners.length > 0">
                                        <tr>  
                                            <td>{{b}}<span class="team_b"></span></td>
                                            <td>  ({{wallet_balance_b.toFixed(2)}})</td>
                                            <td :id="'rt_'+back1" @click="rate(back1,2)">{{back1}}</td>
                                            <td :id="'rt_'+lay1" @click="rate(lay1,4)">{{lay1}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <!-- <table class="table mt-5">
                                ...
                                <tbody v-if="session.length > 0">
                                  <tr v-for="(sess,i) in session">
                                    <td>{{sess.over}}</td>
                                    <td>{{sess.rate}}</td>
                                    <td class="table-danger" @click="sessionModal(sess.rate,i+1,'no',sess.team_id,sess.no,sess.over)">{{sess.no}}</td>
                                    <td class="table-success" @click="sessionModal(sess.rate,i+3,'yes',sess.team_id,sess.yes,sess.over)">{{sess.yes}}</td>
                                  </tr>
                                </tbody>                             
                            </table> -->
                    </div>
...
<div class="modal fade rateModal" id="rate1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="margin-top:50px;">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="row">
              ...
              <input type="hidden" class="form-control"  id="sess" name="session">
              <input type="hidden" class="form-control"  id="over" name="over">
...
export default {
  ...
  created() {
    ...
    axios.get("api/game", {
      ...
      var back = res.data.result[0].runners[0]['ex']['availableToBack'][0].price;
      var back1 = res.data.result[0].runners[1]['ex']['availableToBack'][0].price;
      var lay = res.data.result[0].runners[0]['ex']['availableToLay'][0].price;
      var lay1 = res.data.result[0].runners[1]['ex']['availableToLay'][0].price;
      

      if(Number.isInteger(back) == false){
        if(back < 2){
         back = parseInt((back-1)*100);
        }
        else{
          back = 0;
        }
      ...
      if(Number.isInteger(back1) == false){
        if(back1 < 2){
         back1 = parseInt((back1-1)*100);
        }
        else{
          back1 = 0;
        }
      }
      if(Number.isInteger(lay) == false){
        if(lay < 2){
         lay = parseInt((lay-1)*100);
        }
        else{
          lay = 0;
        }
      }
      if(Number.isInteger(lay1) == false){
        if(lay1 < 2){
         lay1 = parseInt((lay1-1)*100);
        }
        else{
          lay1 = 0;
        }
  ...
  methods: {
    submit(){
      ...
      var amount = $("#calrat1").val();
        ...
        axios.post("api/betStore",{ 
            ...
            tAmnt:tAmnt,
            betAmnt:betAmnt,
            id:this.id.toString().split(".")[1],
            team_a:this.runners[0].runners[0].selectionId,
            team_b:this.runners[0].runners[1].selectionId,
    ...
    rate(data,value){
      if(data >0){
      $(".rateModal").attr("id","rate_"+data.toString().split(".")[1]);
      $("#rate_"+data.toString().split(".")[1]).modal("show");
      var rate1 = $("#rt_"+data.toString().split(".")[1]).text();
      // $("#rat1").val(rate1);
      $("#rat1").val(data);
      $("#val").val(value);
    ...
    sessionModal(data,value,side,team,sess,over){
      if(data > 0){
